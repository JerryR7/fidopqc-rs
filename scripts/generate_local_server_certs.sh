...
# 檢查服務器 CA 證書是否存在
if [ ! -f "${CERTS_DIR}/server-ca.key" ] || [ ! -f "${CERTS_DIR}/server-ca.crt" ]; then
    echo "生成服務器 CA 證書..."
    # 生成服務器 CA 證書
    ...
    openssl x509 -req -days 1825 -in ${CERTS_DIR}/server-ca.csr -CA ${CERTS_DIR}/root-ca.crt -CAkey ${CERTS_DIR}/root-ca.key -CAcreateserial -out ${CERTS_DIR}/server-ca.crt -extfile ${CERTS_DIR}/openssl.cnf -extensions v3_ca
...
# 檢查傳統 CA 證書是否存在
if [ ! -f "${TRADITIONAL_DIR}/ca.key" ] || [ ! -f "${TRADITIONAL_DIR}/ca.crt" ]; then
    echo "生成傳統 RSA CA 證書..."
    # 生成傳統 RSA CA 證書
...
# 使用配置文件簽署證書
${OPENSSL_PQC} x509 -req -days 365 -in ${HYBRID_DIR}/server_ml_dsa_87.csr -CA ${CERTS_DIR}/server-ca.crt -CAkey ${CERTS_DIR}/server-ca.key -CAcreateserial -out ${HYBRID_DIR}/server_ml_dsa_87.crt -extfile ${CERTS_DIR}/openssl.cnf -extensions server_ext
